...
SSD_mult_CRT <- function(test, effect_sizes, n1 = 15, n2 = 30, ndatasets = 1000, out_specific_ICCs, 
    ...
    if (Bayes_pack == "bain") {
        ...
        required_packages <-  c(required_packages, "BFpack")
    }
    miss <- required_packages[!vapply(required_packages, requireNamespace, FUN.VALUE = logical(1), quietly = TRUE)]
    if (length(miss)) stop("Please install missing packages: ", paste(miss, collapse = ", "))
    
    
    # Functions
    required_functions <- c("multiv_data_generation.R",
                            "aafbf.R", "helpers_functions.R", "print_results.R")
    ...
    #TODO: Add warnings for ICCs adn PMP
...
#         # Effective sample size
#         if (test == "intersection-union") {
#             effective_nH1 <- effective_sample(n1, n2, data_H1$ICCs, n_outcomes)
#             effective_nH1 <- min(effective_nH1)
#             effective_nH2 <- effective_sample(n1, n2, data_H2$ICCs, n_outcomes)
#             effective_nH2 <- min(effective_nH2)
#             effective_nH3 <- effective_sample(n1, n2, data_H3$ICCs, n_outcomes)
#             effective_nH3 <- min(effective_nH3)
#             effective_nH4 <- effective_sample(n1, n2, data_H4$ICCs, n_outcomes)
#             effective_nH4 <- min(effective_nH4)
#             
#         } else if (test == "omnibus") {
#             effective_n <- effective_sample(n1, n2, data_H1$ICCs, n_outcomes)
#             effective_n <- min(effective_n)
#             
#         } else if (test == "homogeneity") {
#             effective_nH1 <- effective_sample(n1, n2, data_H1$ICCs, n_outcomes)
#             effective_nH1 <- min(effective_nH1)
